...
- [NetIntel](https://netintel.dev) - Structured-data + network intelligence for AI agents. Headline endpoint: schema-parse — messy text in, strict typed JSON out (the single most-paid category across 1,500+ real agent payments in production). Plus 60+ pay-per-call endpoints: DNS, WHOIS, SSL/TLS grading, IP reputation, email auth (SPF/DKIM/DMARC), OSINT, cert transparency, and composite domain-risk scoring, alongside LLM tools (sentiment, classify, translate, summarize, moderation). $0.002–$0.05 USDC per call on Base via CDP facilitator. No API keys, no signup. ([llms.txt](https://netintel.dev/llms.txt) | [OpenAPI](https://netintel.dev/openapi.json) | [Discovery](https://netintel.dev/.well-known/x402) | [Agent Card](https://netintel.dev/.well-known/agent-card.json) | [MCP Server](https://www.npmjs.com/package/netintel-mcp))
